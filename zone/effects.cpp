...
void EntityList::AESpell(Mob *caster, Mob *center, uint16 spell_id, bool affect_caster, int16 resist_adjust)
{
	...
	for (auto it = mob_list.begin(); it != mob_list.end(); ++it) {
		...
		if (bad) {
			...
			{
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Attempting to cast a detrimental AE spell/song on a player.");
			...
			if (caster->IsAttackAllowed(curmob, true))
			{
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Attempting to cast a beneficial AE spell/song on a NPC.");
				caster->Message_StringID(MT_SpellFailure, SPELL_NO_HOLD);
				continue;
			}
