...
#define _EDLEN	200
	...
	if(c_override)
	{
		...
		if(durat > 0)
		{
			...
			{
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Spell: %d stacking error from item click.", spell_id);
				return false;
			}
		}
		else
	...
	else
	{
		if(IsBuffSpell(spell_id)){
			...
			if(buffslot == -1)	// stacking failure
			{
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Spell: %d stacking error.", spell_id);
				return false;
			}
		} else {
	...
	for (i = 0; i < EFFECT_COUNT; i++)
	{
		...
		switch(effect)
		{
			...
			case SE_TossUp:
			{
				...
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "TossUp Spell: %d has a pushback (%0.1f) pushup (%0.1f) component.", spell_id, spells[spell_id].pushback, toss_amt);
