...
#define _EDLEN	200
	...
	if(c_override)
	{
		...
		if(durat > 0)
		{
			...
			{
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Spell: %d stacking error from item click.", spell_id);
				return false;
			}
		}
		else
	...
	else
	{
		if(IsBuffSpell(spell_id)){
			...
			if(buffslot == -1)	// stacking failure
			{
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Spell: %d stacking error.", spell_id);
				return false;
			}
		} else {
	...
	for (i = 0; i < EFFECT_COUNT; i++)
	{
		...
		switch(effect)
		{
			...
			case SE_InvisVsAnimals:
			{
				...
				SetInvisible(INVIS_VSANIMAL, false);
				break;
			}
			...
			case SE_InvisVsUndead:
			{
				...
				SetInvisible(INVIS_VSUNDEAD, false);
				break;
			}
			case SE_SeeInvis:
			{
			...
			case SE_TossUp:
			{
				...
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "TossUp Spell: %d has a pushback (%0.1f) pushup (%0.1f) component.", spell_id, spells[spell_id].pushback, toss_amt);
	...
	for (int buffs_i = 0; buffs_i < buff_count; ++buffs_i)
	{
		if (buffs[buffs_i].spellid != SPELL_UNKNOWN)
		{
			...
			if(IsClient())
			{
				...
				{
					CastToClient()->SendBuffDurationPacket(buffs[buffs_i].spellid, buffs[buffs_i].ticsremaining, buffs[buffs_i].casterlevel, buffs_i);
