	...
	for (int i=0; i < EFFECT_COUNT; i++)
	{
		...
		switch (spells[buffs[slot].spellid].effectid[i])
		{
			...
			case SE_Charm:
			{
				...
				if(tempmob)
				{
					if(tempmob->GetTarget() && tempmob->GetTarget() == this && (!tempmob->IsClient() || tempmob->IsAIControlled()))
...
int16 Client::GetFocusEffect(focusType type, uint16 spell_id) {
	...
	if (itembonuses.FocusEffects[type]){
		...
		if ((type != focusReagentCost && type != focusSpellHaste) && ((rand_effectiveness && UsedItem) || (realTotal != 0 && UsedItem)))
		{
			// focusReagentCost is already handled in spells.cpp line 1108
