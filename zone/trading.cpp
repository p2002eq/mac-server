						...
						if (free_slot != INVALID_INDEX) 
						{
							if (other->PutItemInInventory(free_slot, *inst, true)) 
							{
									...
									++trade_bag_slot)
								{
									...
									if (inst) {
									Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Giving item in container %s (%d) in slot %d to %s", inst->GetItem()->Name, inst->GetItem()->ID, trade_bag_slot, other->GetName());
	
										if (inst->GetItem()->NoDrop != 0 || Admin() >= RuleI(Character, MinStatusForNoDropExemptions) || RuleI(World, FVNoDropFlag) == 1 || other == this) 
										{
											int16 free_bag_slot = other->GetInv().CalcSlotId(free_slot, bagidx);
											Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Free slot is: %i. Slot bag is in: %i Index is: %i", free_bag_slot, free_slot, bagidx);
											if (free_bag_slot != INVALID_INDEX) {
												if (other->PutItemInInventory(free_bag_slot, *inst, true)) 
												{
													Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Container item %s (%d) successfully transferred, deleting from trade slot.", inst->GetItem()->Name, inst->GetItem()->ID);
												}

												else {
													Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Transfer of container item %s (%d) to %s failed, returning to giver.", inst->GetItem()->Name, inst->GetItem()->ID, other->GetName());
													dropitem = true;
												}
											}
											else {
												Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "%s's inventory is full, returning container item %s (%d) to giver.", other->GetName(), inst->GetItem()->Name, inst->GetItem()->ID);
												dropitem = true;
											}
										}
										else {
											Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Container item %s (%d) is NoDrop, returning to giver.", inst->GetItem()->Name, inst->GetItem()->ID);
...
ItemInst* Client::FindTraderItemByID(int32 ItemID){
	...
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::FindTraderItemByID Couldn't find item! Item No. was %i", ItemID);
...
void Client::NukeTraderItem(uint16 Slot,int16 Charges,uint16 Quantity,Client* Customer,uint16 TraderSlot, int SerialNumber, uint32 SellerID) {
	...
	if(!Customer) return;
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "NukeTraderItem(Slot %i, Charges %i, Quantity %i TraderSlot %i", Slot, Charges, Quantity, TraderSlot);
	if(Quantity < Charges) {
	...
	else
	{
		...
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Telling customer to remove item %i with %i charges.", SerialNumber, Charges);
...
void Client::BuyTraderItem(TraderBuy_Struct* tbs,Client* Trader,const EQApplicationPacket* app){
	...
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Buyitem: Name: %s, IsStackable: %i, Requested Quantity: %i, Charges on Item %i Price: %i Slot: %i Price per item: %i",
	...
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Actual quantity that will be traded is %i for cost: %i", outtbs->Quantity, outtbs->Price);
	...
	if(NewPrice == 0) {
		...
		// If the new price is 0, remove the item(s) from the window.
			...
			for(int i = 0; i < 80; i++) {
				if(gis->ItemID[i] == ItemID) {
					tdis->itemslot = i;
					Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Telling customer to remove item %i with %i charges.", ItemID, Charges);
...
void Client::HandleTraderPriceUpdate(const EQApplicationPacket *app) {
	...
	if(IDOfItemToUpdate == 0) {
		...
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Unable to find item to update price for. Rechecking trader satchels. If you removed an item while in trader mode, this message can be ignored!");
