...
void Client::RangedAttack(Mob* other, bool CanDoubleAttack) {
	...
	logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Combat, "Shooting %s with bow %s (%d) and arrow %s (%d)", GetTarget()->GetName(), RangeItem->Name, RangeItem->ID, AmmoItem->Name, AmmoItem->ID);
	...
	range *= range;
	float dist = ComparativeDistanceNoZ(m_Position, GetTarget()->GetPosition());
	if(dist > range) {
...
void NPC::RangedAttack(Mob* other)
{
	...
	for(int i = 0; i < attacks; ++i) {
		...
		if (ComparativeDistanceNoZ(m_Position, other->GetPosition()) > max_range)
			return;
		else if(ComparativeDistanceNoZ(m_Position, other->GetPosition()) < (min_range * min_range))
			return;
...
void Client::ThrowingAttack(Mob* other, bool CanDoubleAttack) { //old was 51
	...
	logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Combat, "Throwing %s (%d) at %s", item->Name, item->ID, GetTarget()->GetName());
