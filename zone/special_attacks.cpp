...
void Client::RangedAttack(Mob* other, bool CanDoubleAttack) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Combat, "Shooting %s with bow %s (%d) and arrow %s (%d)", GetTarget()->GetName(), RangeItem->Name, RangeItem->ID, AmmoItem->Name, AmmoItem->ID);
...
void NPC::RangedAttack(Mob* other)
{
	...
	for(int i = 0; i < attacks; ++i) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Combat, "Calculated bow range to be %.1f", max_range);
		max_range *= max_range;
...
void Client::ThrowingAttack(Mob* other, bool CanDoubleAttack) { //old was 51
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Combat, "Throwing %s (%d) at %s", item->Name, item->ID, GetTarget()->GetName());
