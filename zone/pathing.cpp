...
PathManager* PathManager::LoadPathFile(const char* ZoneName)
{
	...
	if((PathFile = fopen(ZonePathFileName, "rb")))
	{
		...
		{
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Status, "Path File %s loaded.", ZonePathFileName);

		}
		else
		{
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Path File %s failed to load.", ZonePathFileName);
			safe_delete(Ret);
		}
		fclose(PathFile);
	}
	...
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Path File %s not found.", ZonePathFileName);
	}
...
bool PathManager::loadPaths(FILE *PathFile)
{
	...
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Bad Magic String in .path file.");
		return false;
	}

	fread(&Head, sizeof(Head), 1, PathFile);

	Log.Out(EQEmuLogSys::General, EQEmuLogSys::Status, "Path File Header: Version %ld, PathNodes %ld",
				(long)Head.version, (long)Head.PathNodeCount);

	if(Head.version != 2)
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unsupported path file version.");
		return false;
	}
	...
	for(uint32 i = 0; i < Head.PathNodeCount; ++i)
	{
		for(uint32 j = 0; j < PATHNODENEIGHBOURS; ++j)
		{
			...
			{
				Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Path Node %i, Neighbour %i (%i) out of range.", i, j, PathNodes[i].Neighbours[j].id);
...
{ 
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "FindRoute from node %i to %i", startID, endID);
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Unable to find a route.");
	return Route;
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "FindRoute(%8.3f, %8.3f, %8.3f, %8.3f, %8.3f, %8.3f)", Start.x, Start.y, Start.z, End.x, End.y, End.z);
	...
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Checking Reachability of Node %i from Start Position.", PathNodes[(*Iterator).id].id);
	...
	if(ClosestPathNodeToStart <0 ) {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "No LOS to any starting Path Node within range.");
		return noderoute;
	}

	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Closest Path Node To Start: %2d", ClosestPathNodeToStart);
	...
	for(auto Iterator = SortedByDistance.begin(); Iterator != SortedByDistance.end(); ++Iterator)
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Checking Reachability of Node %i from End Position.", PathNodes[(*Iterator).id].id);
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, " (%8.3f, %8.3f, %8.3f) to (%8.3f, %8.3f, %8.3f)",
			End.x, End.y, End.z,
	...
	if(ClosestPathNodeToEnd < 0) {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "No LOS to any end Path Node within range.");
		return noderoute;
	}

	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Closest Path Node To End: %2d", ClosestPathNodeToEnd);
...
glm::vec3 Mob::UpdatePath(float ToX, float ToY, float ToZ, float Speed, bool &WaypointChanged, bool &NodeReached)
{
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "UpdatePath. From(%8.3f, %8.3f, %8.3f) To(%8.3f, %8.3f, %8.3f)", From.x, From.y, From.z, To.x, To.y, To.z);
	...
	if(From == PathingLastPosition)
	{
		...
		{
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "appears to be stuck. Teleporting them to next position.", GetName());
	...
	if(Route.size() > 0)
	{
		...
		if(SameDestination)
		{
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Still pathing to the same destination.");
			...
			if(NodeLoc == From)
			{
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Arrived at node %i", NextNode);
				...
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Route size is %i", RouteSize);
				...
				{
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Checking distance to target.");
						...
						&& (ABS(From.z - To.z) <= RuleR(Pathing, ZDiffThreshold)))
					{
						...
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "NoLOS");

						if((PathingLOSState == HaveLOS) && zone->pathing->NoHazards(From, To))
						{
							Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  No hazards. Running directly to target.");
							Route.clear();
				...
				if(Route.size() > 0)
				{
					...
					if(NextNode == -1)
					{
						...
						if(Route.size() == 0)
						{
							Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Missing node after teleport.");
							return To;
						}
						...
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  TELEPORTED to %8.3f, %8.3f, %8.3f\n", NodeLoc.x, NodeLoc.y, NodeLoc.z);
					...
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Now moving to node %i", NextNode);
				...
				else
				{
					...
					// to the destination
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Reached end of node path, running direct to target.");
		...
		else
		{
			...
			//
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Target has changed position.");
			...
			if(static_cast<int>(Route.size()) <= RuleI(Pathing, RouteUpdateFrequencyNodeCount))
			{
				...
				{
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Short route update timer not yet expired.");
					return zone->pathing->GetPathNodeCoordinates(Route.front());
				}
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Short route update timer expired.");
			}
			else
			{
				if(!PathingRouteUpdateTimerLong->Check())
				{
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Long route update timer not yet expired.");
					return zone->pathing->GetPathNodeCoordinates(Route.front());
				}
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Long route update timer expired.");
			}
			...
			{
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Unable to find path node for new destination. Running straight to target.");
				Route.clear();
				return To;
			}
			...
			{
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Same destination Node (%i). Continue with current path.", DestinationPathNode);
				...
				if(NodeLoc == From)
				{
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Arrived at node %i, moving to next one.\n", Route.front());
					...
					{
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Reached end of path grid. Running direct to target.");
						return To;
					}
				}
				return NodeLoc;
			...
			{
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Target moved. End node is different. Clearing route.");
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Our route list is empty.");

	if((SameDestination) && !PathingLOSCheckTimer->Check())
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Destination same as before, LOS check timer not reached. Returning To.");
		return To;
	}
		...
		&& (ABS(From.z - To.z) <= RuleR(Pathing, ZDiffThreshold)))
	{
		...
		if((PathingLOSState == HaveLOS) && zone->pathing->NoHazards(From, To))
		{
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Target is reachable. Running directly there.");
			return To;
		}
	}
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Calculating new route to target.");
	...
	if(Route.size() == 0)
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  No route available, running direct.");
	...
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  Probable loop detected. Same destination and Route.front() == PathingLastNodeVisited.");
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  New route determined, heading for node %i", Route.front());
...
bool PathManager::NoHazards(glm::vec3 From, glm::vec3 To)
{
	...
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  HAZARD DETECTED moving from %8.3f, %8.3f, %8.3f to %8.3f, %8.3f, %8.3f. Z Change is %8.3f",
	...
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "No HAZARD DETECTED moving from %8.3f, %8.3f, %8.3f to %8.3f, %8.3f, %8.3f. Z Change is %8.3f",
			From.x, From.y, From.z, MidPoint.x, MidPoint.y, MidPoint.z, NewZ - From.z);
	}
...
bool PathManager::NoHazardsAccurate(glm::vec3 From, glm::vec3 To)
{
	...
	do
	{
		...
		{
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  HAZARD DETECTED moving from %8.3f, %8.3f, %8.3f to %8.3f, %8.3f, %8.3f. Best Z %8.3f, Z Change is %8.3f",
		...
		if (zone->watermap)
		{
			...
			if (zone->watermap->InLiquid(testPointNewZ))
			{
				...
				if (best_z2 == -999990)
				{
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  HAZARD DETECTED, really deep water/lava!");
					return false;
				}
				else
				{
					...
					{
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  HAZARD DETECTED, water is fairly deep at %8.3f units deep", ABS(NewZ - best_z2));
						return false;
					}
					else
					{
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "  HAZARD NOT DETECTED, water is shallow at %8.3f units deep", ABS(NewZ - best_z2));
					}
				}
			}
			else
			{
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Hazard point not in water or lava!");
			}
		}
		else
		{
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "No water map loaded for hazards!");
		}
...
void PathManager::OpenDoors(int Node1, int Node2, Mob *ForWho)
{
	...
	for(int i = 0; i < PATHNODENEIGHBOURS; ++i)
	{
		...
		if(PathNodes[Node1].Neighbours[i].DoorID >= 0)
		{
			...
			if(d && !d->IsDoorOpen() )
			{
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "Opening door %i for %s", PathNodes[Node1].Neighbours[i].DoorID, ForWho->GetName());
