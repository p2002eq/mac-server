	...
	if(Connected() || IsLD())
	{
		...
		if (bardsong_timer.Check() && bardsong != 0) {
			...
			if (song_target == nullptr || (IsCharmSpell(bardsong) && song_target->IsCharmed()))
			{
				...
				{
					Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Attempting to apply a detrimental song pulse on a player.");
...
void Client::OPGMTrainSkill(const EQApplicationPacket *app)
{
	...
	if (gmskill->skillbank == 0x01)
	{
		...
		{
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Combat, "Wrong Training Skill (languages)");
			DumpPacket(app);
			return;
		}
	...
	else if (gmskill->skillbank == 0x00)
	{
		...
		{
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Combat, "Wrong Training Skill (abilities)" );
			DumpPacket(app);
			return;
		}
		...
		if(skilllevel == 0 || (skilllevel == 254 && SkillTrainLevel(skill, GetClass()) <= GetLevel())) {
			...
			if (t_level == 0)
			{
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Combat, "Tried to train a new skill %d which is invalid for this race/class.", skill);
				return;
			}
