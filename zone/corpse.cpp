...
bool Corpse::Process() {
	...
	if (corpse_graveyard_timer.Check()) {
		if (zone->HasGraveyard()) {
			...
			safe_delete(pack);
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Moved %s player corpse to the designated graveyard in zone %s.", this->GetName(), database.GetZoneName(zone->graveyard_zoneid()));
			corpse_db_id = 0;
		}
	...
	if (corpse_decay_timer.Check()) {
		...
		if (!RuleB(Zone, EnableShadowrest)){
			...
			if (database.BuryCharacterCorpse(corpse_db_id)) {
				...
				player_corpse_depop = true;
				corpse_db_id = 0;
				Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Tagged %s player corpse has burried.", this->GetName());
			}
			else {
				Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to bury %s player corpse.", this->GetName());
				return true;
			}
		}
		corpse_decay_timer.Disable();
		...
		if(IsPlayerCorpse() && (char_id == client->CharacterID() || client->GetGM())) {
			...
			if (IsPlayerCorpse() && i == 0 && itemlist.size() > 0) { // somehow, player corpse contains items, but client doesn't see them...
				...
				for (; cur != end; ++cur) {
					ServerLootItem_Struct* item_data = *cur;
					item = database.GetItem(item_data->item_id);
					Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Corpse Looting: %s was not sent to client loot window (corpse_dbid: %i, charname: %s(%s))", item->Name, GetCorpseDBID(), client->GetName(), client->GetGM() ? "GM" : "Owner");
