...
int Client::HandlePacket(const EQApplicationPacket *app)
{
	if(is_log_enabled(CLIENT__NET_IN_TRACE)) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Client_Server_Packet, "Dispatch opcode: %s", buffer);
	}
...
#if EQDEBUG >= 10
			//Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "HandlePacket() Opcode error: Unexpected packet during CLIENT_CONNECTING: opcode:"
			//	" %s (#%d eq=0x%04x), size: %i", OpcodeNames[opcode], opcode, 0, app->size);
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error,"Received unknown EQApplicationPacket during CLIENT_CONNECTING");
			//DumpPacket(app);
#endif
			break;
		}
	...
	case CLIENT_CONNECTED: {
		...
		if(p == nullptr) {
			...
			char buffer[64]; 
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Client_Server_Packet, "Unhandled incoming opcode: %s - 0x%04x", OpcodeManager::EmuToName(app->GetOpcode()), app->GetOpcode());
	...
	case ZONING:
		break;
	default:
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Unknown client_state: %d\n", client_state);
		break;
	}
...
void Client::CompleteConnect()
{
	...
	//enforce some rules..
	if (!CanBeInZone()) {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "[CLIENT] Kicking char from zone, not allowed here");
	...
	if(GetClientVersion() == EQClientMac)
	{
		...
		if(GetGM())
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Client_Server_Packet, "Client version is: %s. The client type is: %s.", string.c_str(), type.c_str());
	...
	else
	{
		...
		if(GetGM())
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Client_Server_Packet, "Client version is: %s.", string.c_str());
	}
...
void Client::Handle_Connect_OP_ClientError(const EQApplicationPacket *app)
{
	if (app->size != sizeof(ClientError_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size on OP_ClientError: Expected %i, Got %i",
...
void Client::Handle_Connect_OP_ReqNewZone(const EQApplicationPacket *app)
{
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Zone_Server, "NewZone data for %s (%i) successfully sent.", zone->newzone_data.zone_short_name, zone->newzone_data.zone_id);
...
void Client::Handle_Connect_OP_SetServerFilter(const EQApplicationPacket *app)
{
	if (app->size != sizeof(SetServerFilter_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Received invalid sized OP_SetServerFilter");
		DumpPacket(app);
		return;
	}
...
void Client::Handle_Connect_OP_TGB(const EQApplicationPacket *app)
{
	if (app->size != sizeof(uint32)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size on OP_TGB: Expected %i, Got %i",
...
void Client::Handle_Connect_OP_ZoneEntry(const EQApplicationPacket *app)
{
	...
	*/
	...
	if (!zone->GetAuth(ip, cze->char_name, &WID, &account_id, &character_id, &admin, lskey, &tellsoff, &versionbit)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "GetAuth() returned false kicking client");
		if (client != 0) {
			client->Save();
			client->Kick();
		}
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Zone_Server, "ClientVersionBit is: %i", ClientVersionBit);
	...
	if (client != 0) {
		...
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Ghosting client: Account ID:%i Name:%s Character:%s IP:%s",
	...
	if (!p_timers.Load(&database)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to load ability timers from the database for %s (%i)!", GetCleanName(), CharacterID());
	}
	...
	if (loaditems) { /* Dont load if a length error occurs */
		...
		for (slot_id = EmuConstants::CURSOR_BAG_BEGIN; slot_id <= EmuConstants::CURSOR_BAG_END; slot_id++) {
			...
			if (inst){
				...
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Sending cursor bag with items.");
				break;
			}
		}
	}
...
void Client::Handle_OP_AAAction(const EQApplicationPacket *app)
{
	...
	else if (strncmp((char *)app->pBuffer, "buy ", 4) == 0)
	{
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::AA, "Buying: EmuaaID: %i, MacaaID: %i, action: %i, exp: %i", action->ability, aa, action->action, action->exp_value);
		BuyAA(action);
	}
	else if (strncmp((char *)app->pBuffer, "activate ", 9) == 0)
	{
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::AA, "Activating AA %d", action->ability);
...
void Client::Handle_OP_Animation(const EQApplicationPacket *app)
{
	if (app->size != sizeof(Animation_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Received invalid sized "
...
void Client::Handle_OP_ApplyPoison(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(ApplyPoison_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_ApplyPoison, size=%i, expected %i", app->size, sizeof(ApplyPoison_Struct));
		DumpPacket(app);
		return;
	}
	...
	if (!IsPoison)
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Item used to cast spell effect from a poison item was missing from inventory slot %d "
	...
	else if (GetClass() == ROGUE)
	{
		if ((PrimaryWeapon && PrimaryWeapon->GetItem()->ItemType == ItemType1HPiercing) ||
			...
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Chance to Apply Poison was %f. Roll was %f. Result is %u.", SuccessChance, ChanceRoll, ApplyPoisonSuccessResult);
		}
	}
...
void Client::Handle_OP_Assist(const EQApplicationPacket *app)
{
	if (app->size != sizeof(EntityId_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_Assist expected %i got %i", sizeof(EntityId_Struct), app->size);
		return;
	}
...
void Client::Handle_OP_AutoAttack(const EQApplicationPacket *app)
{
	...
	if (app->size != 4) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "OP size error: OP_AutoAttack expected:4 got:%i", app->size);
		return;
	}
...
void Client::Handle_OP_BazaarSearch(const EQApplicationPacket *app)
{
	...
	if (app->size == sizeof(BazaarSearch_Struct)) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Malformed BazaarSearch_Struct packe, Action %it received, ignoring...");
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Malformed BazaarSearch_Struct packet received, ignoring...\n");
	}
...
void Client::Handle_OP_Bind_Wound(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(BindWound_Struct)){
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Size mismatch for Bind wound packet");
		DumpPacket(app);
	}
	...
	if (!bindmob){
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Bindwound on non-exsistant mob from %s", this->GetName());
	}
	else {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "BindWound in: to:\'%s\' from=\'%s\'", bindmob->GetName(), GetName());
...
void Client::Handle_OP_BoardBoat(const EQApplicationPacket *app)
{
	...
	if (app->size <= 7 || app->size > 20) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_BoardBoad. Expected greater than 7 no greater than 20, got %i", app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_Buff(const EQApplicationPacket *app)
{
	...
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Size mismatch in OP_Buff. expected %i got %i", sizeof(SpellBuffFade_Struct), app->size);
		DumpPacket(app);
		return;
	}
	SpellBuffFade_Struct* sbf = (SpellBuffFade_Struct*)app->pBuffer;
	uint32 spid = sbf->spellid;
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Client requested that buff with spell id %d be canceled. effect: %d slotid: %d slot: %d duration: %d level: %d", sbf->spellid, sbf->effect, sbf->slotid, sbf->slot, sbf->duration, sbf->level);
...
void Client::Handle_OP_CancelTrade(const EQApplicationPacket *app)
{
	if (app->size != sizeof(CancelTrade_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_CancelTrade, size=%i, expected %i", app->size, sizeof(CancelTrade_Struct));
		return;
	}
	Mob* with = trade->With();
...
#ifdef _EQDEBUG
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: %u %i", (uint8)castspell->cs_unknown[0], castspell->cs_unknown[0]);
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: %u %i", (uint8)castspell->cs_unknown[1], castspell->cs_unknown[1]);
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: %u %i", (uint8)castspell->cs_unknown[2], castspell->cs_unknown[2]);
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: %u %i", (uint8)castspell->cs_unknown[3], castspell->cs_unknown[3]);
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: 32 %p %u", &castspell->cs_unknown, *(uint32*)castspell->cs_unknown);
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: 32 %p %i", &castspell->cs_unknown, *(uint32*)castspell->cs_unknown);
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: 16 %p %u %u", &castspell->cs_unknown, *(uint16*)castspell->cs_unknown, *(uint16*)castspell->cs_unknown + sizeof(uint16));
...
#endif
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "OP CastSpell: slot=%d, spell=%d, target=%d, inv=%lx", castspell->slot, castspell->spell_id, castspell->target_id, (unsigned long)castspell->inventoryslot);
	...
	else if ((castspell->slot == USE_ITEM_SPELL_SLOT))	// ITEM or POTION cast
	{
		...
		if (castspell->inventoryslot == INVALID_INDEX) {
			if (!UseDiscipline(castspell->spell_id, castspell->target_id)) {
				Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Unknown ability being used by %s, spell being cast is: %i\n", GetName(), castspell->spell_id);
				InterruptSpell(castspell->spell_id);
			}
			return;
		}
...
void Client::Handle_OP_ClickDoor(const EQApplicationPacket *app)
{
	if (app->size != sizeof(ClickDoor_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_ClickDoor, size=%i, expected %i", app->size, sizeof(ClickDoor_Struct));
		return;
	}
...
void Client::Handle_OP_ClickObject(const EQApplicationPacket *app) 
{
	if (app->size != sizeof(ClickObject_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size on ClickObject_Struct: Expected %i, Got %i",
...
void Client::Handle_OP_ClickObjectAction(const EQApplicationPacket *app)
{
	...
	if (entity && entity->IsObject()) {
		...
		if (oos->open == 0) {
			...
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unsupported action %d in OP_ClickObjectAction", oos->open);
		}
	}
	else {
...
void Client::Handle_OP_ClientError(const EQApplicationPacket *app)
{
	...
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Client error: %s", error->character_name);
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error message:%s", error->message);
	return;
}
...
void Client::Handle_OP_ClientUpdate(const EQApplicationPacket *app)
{
	...
	&& app->size != (sizeof(SpawnPositionUpdate_Struct)+1)
		...
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "OP size error: OP_ClientUpdate expected:%i got:%i", sizeof(SpawnPositionUpdate_Struct), app->size);
		return;
	}
...
void Client::Handle_OP_Consider(const EQApplicationPacket *app)
{
	...
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in Consider expected %i got %i", sizeof(Consider_Struct), app->size);
		return;
	}
...
void Client::Handle_OP_ConsiderCorpse(const EQApplicationPacket *app)
{
	if (app->size != sizeof(Consider_Struct))
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in Consider corpse expected %i got %i", sizeof(Consider_Struct), app->size);
		return;
	}
	Consider_Struct* conin = (Consider_Struct*)app->pBuffer;
...
void Client::Handle_OP_Consume(const EQApplicationPacket *app)
{
	...
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "OP size error: OP_Consume expected:%i got:%i", sizeof(Consume_Struct), app->size);
		return;
	}
	...
	if (myitem == nullptr) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Consuming from empty slot %d", pcs->slot);
		return;
	}
	...
	if (pcs->type == 0x01) {
		...
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "OP_Consume: unknown type, type:%i", (int)pcs->type);
		return;
	}
...
void Client::Handle_OP_ControlBoat(const EQApplicationPacket *app)
{
	if (app->size != sizeof(OldControlBoat_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_ControlBoat, size=%i, expected %i", app->size, sizeof(OldControlBoat_Struct));
		return;
	}
...
void Client::Handle_OP_Emote(const EQApplicationPacket *app)
{
	if (app->size != sizeof(OldEmote_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Received invalid sized ""OP_Emote: got %d, expected %d", app->size, sizeof(OldEmote_Struct));
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_EnvDamage(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(EnvDamage2_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Received invalid sized OP_EnvDamage: got %d, expected %d", app->size,
...
void Client::Handle_OP_FaceChange(const EQApplicationPacket *app)
{
	if (app->size != sizeof(FaceChange_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size for OP_FaceChange: Expected: %i, Got: %i",
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GetGuildsList");
	SendPlayerGuild();
}
...
void Client::Handle_OP_GMEmoteZone(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(GMEmoteZone_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_GMEmoteZone, size=%i, expected %i", app->size, sizeof(GMEmoteZone_Struct));
		return;
	}
...
void Client::Handle_OP_GMEndTraining(const EQApplicationPacket *app)
{
	if (app->size != sizeof(GMTrainEnd_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_GMEndTraining expected %i got %i", sizeof(GMTrainEnd_Struct), app->size);
		DumpPacket(app);
		return;
	}
	OPGMEndTraining(app);
...
void Client::Handle_OP_GMFind(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(GMSummon_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_GMFind, size=%i, expected %i", app->size, sizeof(GMSummon_Struct));
		return;
	}
	//Break down incoming
...
void Client::Handle_OP_GMHideMe(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(SpawnAppearance_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_GMHideMe, size=%i, expected %i", app->size, sizeof(SpawnAppearance_Struct));
		return;
	}
...
void Client::Handle_OP_GMKill(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(GMKill_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_GMKill, size=%i, expected %i", app->size, sizeof(GMKill_Struct));
		return;
	}
...
void Client::Handle_OP_GMNameChange(const EQApplicationPacket *app)
{
	if (app->size != sizeof(GMName_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_GMNameChange, size=%i, expected %i", app->size, sizeof(GMName_Struct));
		return;
	}
	...
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::Status, "GM(%s) changeing players name. Old:%s New:%s", GetName(), gmn->oldname, gmn->newname);
...
void Client::Handle_OP_GMSearchCorpse(const EQApplicationPacket *app)
{
	...
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "OP_GMSearchCorpse size lower than expected: got %u expected at least %u",
...
void Client::Handle_OP_GMTraining(const EQApplicationPacket *app)
{
	if (app->size != sizeof(OldGMTrainee_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_GMTraining expected %i got %i", sizeof(OldGMTrainee_Struct), app->size);
		DumpPacket(app);
		return;
	}
	OPGMTraining(app);
...
void Client::Handle_OP_GMTrainSkill(const EQApplicationPacket *app)
{
	if (app->size != sizeof(GMSkillChange_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_GMTrainSkill expected %i got %i", sizeof(GMSkillChange_Struct), app->size);
		DumpPacket(app);
		return;
	}
	OPGMTrainSkill(app);
...
void Client::Handle_OP_GMZoneRequest2(const EQApplicationPacket *app)
{
	...
	if (app->size < sizeof(uint32)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "OP size error: OP_GMZoneRequest2 expected:%i got:%i", sizeof(uint32), app->size);
		return;
	}
...
void Client::Handle_OP_GroupCancelInvite(const EQApplicationPacket *app)
{
	if (app->size != sizeof(GroupCancel_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size for OP_GroupCancelInvite: Expected: %i, Got: %i",
...
void Client::Handle_OP_GroupDisband(const EQApplicationPacket *app)
{
	if (app->size != sizeof(GroupGeneric_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size for GroupGeneric_Struct: Expected: %i, Got: %i",
			sizeof(GroupGeneric_Struct), app->size);
		return;
	}

	Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Member Disband Request from %s\n", GetName());
	...
	else
	{
		...
		{
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Failed to remove player from group. Unable to find player named %s in player group", gd->name2);
		}
	}
	return;
}
...
{
	if (app->size != sizeof(GroupGeneric_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size for OP_GroupFollow: Expected: %i, Got: %i",
			sizeof(GroupGeneric_Struct), app->size);
		return;
	}
...
void Client::Handle_OP_GroupInvite2(const EQApplicationPacket *app)
{
	if (app->size != sizeof(GroupInvite_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size for OP_GroupInvite: Expected: %i, Got: %i",
...
void Client::Handle_OP_GroupUpdate(const EQApplicationPacket *app)
{
	...
	case groupActMakeLeader:
	{
		...
		if (newleader && group) {
			...
			if (group->IsLeader(this))
				...
				Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Group /makeleader request originated from non-leader member: %s", GetName());
				DumpPacket(app);
			}
		}
		break;
	...
	default:
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Received unhandled OP_GroupUpdate requesting action %u", gu->action);
		DumpPacket(app);
		return;
	}
	}
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildDelete");

	if (!IsInAGuild() || !guild_mgr.IsGuildLeader(GuildID(), CharacterID()))
		Message(0, "You are not a guild leader or not in a guild.");
	else {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Deleting guild %s (%d)", guild_mgr.GetGuildName(GuildID()), GuildID());
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildInvite");
		...
		if (invitee->IsClient()) {
			...
			if (client->GuildID() == GuildID()) {
				...
				if (gc->officer < client->GuildRank()) {
					...
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "%s (%d) is demoting %s (%d) to rank %d in guild %s (%d)",
						GetName(), CharacterID(),
					...
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "%s (%d) is asking to promote %s (%d) to rank %d in guild %s (%d)",
						GetName(), CharacterID(),
						client->GetName(), client->CharacterID(),
						gc->officer,
						guild_mgr.GetGuildName(GuildID()), GuildID());
					...
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Sending OP_GuildInvite for promotion to %s, length %d", client->GetName(), app->size);
				...
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Inviting %s (%d) into guild %s (%d)",
					client->GetName(), client->CharacterID(),
					guild_mgr.GetGuildName(GuildID()), GuildID());
				...
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Sending OP_GuildInvite for invite to %s, length %d", client->GetName(), app->size);
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildInviteAccept");
	...
	if (IsInAGuild() && gj->response == GuildRank())
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Guild Invite Accept: guild %d, response %d, inviter %s, person %s",
		...
		if (gj->guildeqid == GuildID()) {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Changing guild rank of %s (%d) to rank %d in guild %s (%d)",
				GetName(), CharacterID(),
				gj->response,
				guild_mgr.GetGuildName(GuildID()), GuildID());
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Adding %s (%d) to guild %s (%d) at rank %d",
				GetName(), CharacterID(),
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildLeader");

	if (app->size < 2) {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Invalid length %d on OP_GuildLeader", app->size);
		return;
	}
	...
	if (!IsInAGuild())
		...
		if (newleader) {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Transfering leadership of %s (%d) to %s (%d)",
				guild_mgr.GetGuildName(GuildID()), GuildID(),
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Got OP_GuildPeace of len %d", app->size);
	return;
}

void Client::Handle_OP_GuildRemove(const EQApplicationPacket *app)
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildRemove");
	...
	// we can always remove ourself, otherwise, our rank needs remove permissions
		...
		if (client) {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Removing %s (%d) from guild %s (%d)",
				client->GetName(), client->CharacterID(),
				guild_mgr.GetGuildName(GuildID()), GuildID());
		}
		else {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Removing remote/offline %s (%d) into guild %s (%d)",
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Got OP_GuildWar of len %d", app->size);
	return;
}
...
void Client::Handle_OP_Illusion(const EQApplicationPacket *app)
{
	if (app->size != sizeof(Illusion_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Received invalid sized OP_Illusion: got %d, expected %d", app->size,
...
void Client::Handle_OP_InspectAnswer(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(OldInspectResponse_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_InspectAnswer, size=%i, expected %i", app->size, sizeof(OldInspectResponse_Struct));
		return;
	}
...
void Client::Handle_OP_InspectRequest(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(Inspect_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_InspectRequest, size=%i, expected %i", app->size, sizeof(Inspect_Struct));
		return;
	}
...
void Client::Handle_OP_ItemLinkResponse(const EQApplicationPacket *app) 
{
	if (app->size != sizeof(ItemViewRequest_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "OP size error: OP_ItemLinkResponse expected:%i got:%i", sizeof(ItemViewRequest_Struct), app->size);
		return;
	}
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "%s sent a logout packet.", GetName());
...
void Client::Handle_OP_LootItem(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(LootingItem_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_LootItem, size=%i, expected %i", app->size, sizeof(LootingItem_Struct));
		return;
	}
	/*
...
void Client::Handle_OP_MoveItem(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(MoveItem_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_MoveItem, size=%i, expected %i", app->size, sizeof(MoveItem_Struct));
		return;
	}

	MoveItem_Struct* mi = (MoveItem_Struct*)app->pBuffer;
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Moveitem from_slot: %i, to_slot: %i, number_in_stack: %i", mi->from_slot, mi->to_slot, mi->number_in_stack);
	...
	if (IsValidSlot(mi->from_slot) && IsValidSlot(mi->to_slot)) {
		...
		if (!si)
		{
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "WTF Some shit failed. SwapItem: %i, IsValidSlot (from): %i, IsValidSlot (to): %i", SwapItem(mi), IsValidSlot(mi->from_slot), IsValidSlot(mi->to_slot));
			SwapItemResync(mi);
...
void Client::Handle_OP_PetCommands(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(PetCommand_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_PetCommands, size=%i, expected %i", app->size, sizeof(PetCommand_Struct));
		return;
	}
	char val1[20] = { 0 };
...
void Client::Handle_OP_PetitionCheckIn(const EQApplicationPacket *app)
{
	if (app->size != sizeof(Petition_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_PetitionCheckIn, size=%i, expected %i", app->size, sizeof(Petition_Struct));
		return;
	}
...
void Client::Handle_OP_PetitionDelete(const EQApplicationPacket *app)
{
	if (app->size != sizeof(PetitionUpdate_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_PetitionDelete, size=%i, expected %i", app->size, sizeof(PetitionUpdate_Struct));
		return;
	}
...
void Client::Handle_OP_PickPocket(const EQApplicationPacket *app) 
{
	...
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Size mismatch for Pick Pocket packet");
		DumpPacket(app);
	}
...
void Client::Handle_OP_RaidCommand(const EQApplicationPacket *app)
{
	if (app->size < sizeof(RaidGeneral_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_RaidCommand, size=%i, expected at least %i", app->size, sizeof(RaidGeneral_Struct));
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_RandomReq(const EQApplicationPacket *app)
{
	if (app->size != sizeof(RandomReq_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_RandomReq, size=%i, expected %i", app->size, sizeof(RandomReq_Struct));
		return;
	}
...
void Client::Handle_OP_ReadBook(const EQApplicationPacket *app)
{
	if (app->size != sizeof(BookRequest_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_ReadBook, size=%i, expected %i", app->size, sizeof(BookRequest_Struct));
		return;
	}
...
void Client::Handle_OP_RezzAnswer(const EQApplicationPacket *app)
{
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Received OP_RezzAnswer from client. Pendingrezzexp is %i, action is %s",
...
void Client::Handle_OP_Sacrifice(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(Sacrifice_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_Sacrifice expected %i got %i", sizeof(Sacrifice_Struct), app->size);
		DumpPacket(app);
		return;
	}
	...
	if (!PendingSacrifice) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unexpected OP_Sacrifice reply");
		DumpPacket(app);
		return;
	}
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_SetGuildMOTD");

	if (app->size != sizeof(GuildMOTD_Struct)) {
		// client calls for a motd on login even if they arent in a guild
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Error: app size of %i != size of GuildMOTD_Struct of %zu\n", app->size, sizeof(GuildMOTD_Struct));
		return;
	}
	if (!IsInAGuild()) {
	...
	if (gmotd->motd[0] == 0)
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Client is trying to remove MOTD. This may be intentional but still will be prevented for now.");
		return; //EQMac sends this on login, and it overwrites the existing MOTD. Need to figure out a better way to handle it
	}

	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Setting MOTD for %s (%d) to: %s - %s",
...
void Client::Handle_OP_SetTitle(const EQApplicationPacket *app)
{
	if (app->size != sizeof(SetTitle_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_SetTitle expected %i got %i", sizeof(SetTitle_Struct), app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_Shielding(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(Shielding_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "OP size error: OP_Shielding expected:%i got:%i", sizeof(Shielding_Struct), app->size);
		return;
...
void Client::Handle_OP_ShopPlayerBuy(const EQApplicationPacket *app) 
{
	if (app->size != sizeof(Merchant_Sell_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size on OP_ShopPlayerBuy: Expected %i, Got %i",
...
#if EQDEBUG >= 11
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "%s, purchase item..", GetName());
	DumpPacket(app);
#endif
	...
	if (!stacked && inst) {
		...
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "OP_ShopPlayerBuy: item->ItemClass Unknown! Type: %i", item->ItemClass);
	}
...
void Client::Handle_OP_ShopPlayerSell(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(Merchant_Purchase_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size on OP_ShopPlayerSell: Expected %i, Got %i",
...
void Client::Handle_OP_ShopRequest(const EQApplicationPacket *app) 
{
	if (app->size != sizeof(Merchant_Click_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_ShopRequest, size=%i, expected %i", app->size, sizeof(Merchant_Click_Struct));
		return;
	}
...
void Client::Handle_OP_Split(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(Split_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_Split, size=%i, expected %i", app->size, sizeof(Split_Struct));
		return;
	}
...
void Client::Handle_OP_Surname(const EQApplicationPacket *app)
{
	...
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in Surname expected %i got %i", sizeof(Surname_Struct), app->size);
		return;
	}
...
void Client::Handle_OP_TargetCommand(const EQApplicationPacket *app)
{
	if (app->size != sizeof(ClientTarget_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "OP size error: OP_TargetMouse expected:%i got:%i", sizeof(ClientTarget_Struct), app->size);
		return;
	}
...
void Client::Handle_OP_Trader(const EQApplicationPacket *app) 
{
	...
	if (app->size == sizeof(Trader_ShowItems_Struct))
	{
		...
		case BazaarTrader_EndTransaction: {
			...
			if (c)
				...
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::Handle_OP_Trader: Null Client Pointer");
		...
		default: {
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Unhandled action code in OP_Trader ShowItems_Struct");
			break;
		}
		}
	}
	else if (app->size == sizeof(ClickTrader_Struct) || app->size == sizeof(TraderStatus_Struct))
	{
		...
		else if (app->size == sizeof(TraderStatus_Struct))
		{
			...
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unknown TraderStruct code of: %i\n", ints->Code);
		}
	}
	...
	else if (app->size == sizeof(TraderPriceUpdate_Struct))
	{
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Unknown size for OP_Trader: %i\n", app->size);
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unknown size for OP_Trader: %i\n", app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_TraderBuy(const EQApplicationPacket *app) 
{
	...
	if (app->size == sizeof(TraderBuy_Struct)){
		...
		if (Client* Trader = entity_list.GetClientByID(tbs->TraderID)){
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::Handle_OP_TraderBuy: Null Client Pointer");
		}
	}
	else {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::Handle_OP_TraderBuy: Struct size mismatch");
...
void Client::Handle_OP_TradeRequest(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(TradeRequest_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_TradeRequest, size=%i, expected %i", app->size, sizeof(TradeRequest_Struct));
		return;
	}
...
void Client::Handle_OP_TradeRequestAck(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(TradeRequest_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Wrong size: OP_TradeRequestAck, size=%i, expected %i", app->size, sizeof(TradeRequest_Struct));
		return;
	}
...
void Client::Handle_OP_TraderShop(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(TraderClick_Struct)) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::Handle_OP_TraderShop: Returning due to struct size mismatch");
	...
	if (Trader)
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::Handle_OP_TraderShop: entity_list.GetClientByID(tcs->traderid)"
...
void Client::Handle_OP_TradeSkillCombine(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(NewCombine_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Invalid size for NewCombine_Struct: Expected: %i, Got: %i",
...
void Client::Handle_OP_Translocate(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(Translocate_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_Translocate expected %i got %i", sizeof(Translocate_Struct), app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_LFGCommand(const EQApplicationPacket *app)
{
	if (app->size != sizeof(LFG_Struct)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Invalid size for LFG_Struct: Expected: %i, Got: %i", sizeof(LFG_Struct), app->size);
		return;
	}
	...
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "Invalid LFG value sent. %i", lfg->value);
	}
