...
int Client::HandlePacket(const EQApplicationPacket *app)
{
	if(is_log_enabled(CLIENT__NET_IN_TRACE)) {
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Client_Server_Packet, "Dispatch opcode: %s", buffer);
	}
	...
	case CLIENT_CONNECTED: {
		...
		if(p == nullptr) {
			...
			char buffer[64]; 
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Client_Server_Packet, "Unhandled incoming opcode: %s - 0x%04x", OpcodeManager::EmuToName(app->GetOpcode()), app->GetOpcode());
	...
	case ZONING:
		break;
	default:
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Unknown client_state: %d\n", client_state);
		break;
	}
...
void Client::CompleteConnect()
{
	...
	//enforce some rules..
	if (!CanBeInZone()) {
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::None, "[CLIENT] Kicking char from zone, not allowed here");
	...
	if(GetClientVersion() == EQClientMac)
	{
		...
		if(GetGM())
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Client_Server_Packet, "Client version is: %s. The client type is: %s.", string.c_str(), type.c_str());
	...
	else
	{
		...
		if(GetGM())
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Client_Server_Packet, "Client version is: %s.", string.c_str());
	}
...
void Client::Handle_Connect_OP_ReqNewZone(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Zone_Server, "NewZone data for %s (%i) successfully sent.", zone->newzone_data.zone_short_name, zone->newzone_data.zone_id);
...
void Client::Handle_Connect_OP_ZoneEntry(const EQApplicationPacket *app)
{
	...
	*/
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Zone_Server, "ClientVersionBit is: %i", ClientVersionBit);
	...
	if (loaditems) { /* Dont load if a length error occurs */
		...
		for (slot_id = EmuConstants::CURSOR_BAG_BEGIN; slot_id <= EmuConstants::CURSOR_BAG_END; slot_id++) {
			...
			if (inst){
				...
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Sending cursor bag with items.");
				break;
			}
		}
	}
...
void Client::Handle_OP_AAAction(const EQApplicationPacket *app)
{
	...
	else if (strncmp((char *)app->pBuffer, "buy ", 4) == 0)
	{
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::AA, "Buying: EmuaaID: %i, MacaaID: %i, action: %i, exp: %i", action->ability, aa, action->action, action->exp_value);
		BuyAA(action);
	}
	else if (strncmp((char *)app->pBuffer, "activate ", 9) == 0)
	{
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::AA, "Activating AA %d", action->ability);
...
void Client::Handle_OP_ApplyPoison(const EQApplicationPacket *app) 
{
	...
	if (!IsPoison)
	{
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Item used to cast spell effect from a poison item was missing from inventory slot %d "
	...
	else if (GetClass() == ROGUE)
	{
		if ((PrimaryWeapon && PrimaryWeapon->GetItem()->ItemType == ItemType1HPiercing) ||
			...
			Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Chance to Apply Poison was %f. Roll was %f. Result is %u.", SuccessChance, ChanceRoll, ApplyPoisonSuccessResult);
		}
	}
...
void Client::Handle_OP_Assist(const EQApplicationPacket *app)
{
	if (app->size != sizeof(EntityId_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_Assist expected %i got %i", sizeof(EntityId_Struct), app->size);
		return;
	}
...
void Client::Handle_OP_BoardBoat(const EQApplicationPacket *app)
{
	...
	if (app->size <= 7 || app->size > 20) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_BoardBoad. Expected greater than 7 no greater than 20, got %i", app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_Buff(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Client requested that buff with spell id %d be canceled. effect: %d slotid: %d slot: %d duration: %d level: %d", sbf->spellid, sbf->effect, sbf->slotid, sbf->slot, sbf->duration, sbf->level);
...
#ifdef _EQDEBUG
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: %u %i", (uint8)castspell->cs_unknown[0], castspell->cs_unknown[0]);
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: %u %i", (uint8)castspell->cs_unknown[1], castspell->cs_unknown[1]);
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: %u %i", (uint8)castspell->cs_unknown[2], castspell->cs_unknown[2]);
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: %u %i", (uint8)castspell->cs_unknown[3], castspell->cs_unknown[3]);
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: 32 %p %u", &castspell->cs_unknown, *(uint32*)castspell->cs_unknown);
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: 32 %p %i", &castspell->cs_unknown, *(uint32*)castspell->cs_unknown);
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "cs_unknown2: 16 %p %u %u", &castspell->cs_unknown, *(uint16*)castspell->cs_unknown, *(uint16*)castspell->cs_unknown + sizeof(uint16));
...
#endif
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "OP CastSpell: slot=%d, spell=%d, target=%d, inv=%lx", castspell->slot, castspell->spell_id, castspell->target_id, (unsigned long)castspell->inventoryslot);
	...
	else if ((castspell->slot == USE_ITEM_SPELL_SLOT))	// ITEM or POTION cast
	{
		...
		if (castspell->inventoryslot == INVALID_INDEX) {
			if (!UseDiscipline(castspell->spell_id, castspell->target_id)) {
				Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Unknown ability being used by %s, spell being cast is: %i\n", GetName(), castspell->spell_id);
				InterruptSpell(castspell->spell_id);
			}
			return;
		}
...
void Client::Handle_OP_Consider(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in Consider expected %i got %i", sizeof(Consider_Struct), app->size);
		return;
	}
...
void Client::Handle_OP_ConsiderCorpse(const EQApplicationPacket *app)
{
	if (app->size != sizeof(Consider_Struct))
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in Consider corpse expected %i got %i", sizeof(Consider_Struct), app->size);
		return;
	}
	Consider_Struct* conin = (Consider_Struct*)app->pBuffer;
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GetGuildsList");
	SendPlayerGuild();
}
...
void Client::Handle_OP_GMEndTraining(const EQApplicationPacket *app)
{
	if (app->size != sizeof(GMTrainEnd_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_GMEndTraining expected %i got %i", sizeof(GMTrainEnd_Struct), app->size);
		DumpPacket(app);
		return;
	}
	OPGMEndTraining(app);
...
void Client::Handle_OP_GMSearchCorpse(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "OP_GMSearchCorpse size lower than expected: got %u expected at least %u",
...
void Client::Handle_OP_GMTraining(const EQApplicationPacket *app)
{
	if (app->size != sizeof(OldGMTrainee_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_GMTraining expected %i got %i", sizeof(OldGMTrainee_Struct), app->size);
		DumpPacket(app);
		return;
	}
	OPGMTraining(app);
...
void Client::Handle_OP_GMTrainSkill(const EQApplicationPacket *app)
{
	if (app->size != sizeof(GMSkillChange_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_GMTrainSkill expected %i got %i", sizeof(GMSkillChange_Struct), app->size);
		DumpPacket(app);
		return;
	}
	OPGMTrainSkill(app);
...
void Client::Handle_OP_GroupDisband(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Member Disband Request from %s\n", GetName());
...
void Client::Handle_OP_GroupUpdate(const EQApplicationPacket *app)
{
	...
	case groupActMakeLeader:
	{
		...
		if (newleader && group) {
			...
			if (group->IsLeader(this))
				...
				Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Group /makeleader request originated from non-leader member: %s", GetName());
				DumpPacket(app);
			}
		}
		break;
	...
	default:
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Received unhandled OP_GroupUpdate requesting action %u", gu->action);
		DumpPacket(app);
		return;
	}
	}
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildDelete");

	if (!IsInAGuild() || !guild_mgr.IsGuildLeader(GuildID(), CharacterID()))
		Message(0, "You are not a guild leader or not in a guild.");
	else {
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Deleting guild %s (%d)", guild_mgr.GetGuildName(GuildID()), GuildID());
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildInvite");
		...
		if (invitee->IsClient()) {
			...
			if (client->GuildID() == GuildID()) {
				...
				if (gc->officer < client->GuildRank()) {
					...
					Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "%s (%d) is demoting %s (%d) to rank %d in guild %s (%d)",
						GetName(), CharacterID(),
					...
					Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "%s (%d) is asking to promote %s (%d) to rank %d in guild %s (%d)",
						GetName(), CharacterID(),
						client->GetName(), client->CharacterID(),
						gc->officer,
						guild_mgr.GetGuildName(GuildID()), GuildID());
					...
					Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Sending OP_GuildInvite for promotion to %s, length %d", client->GetName(), app->size);
				...
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Inviting %s (%d) into guild %s (%d)",
					client->GetName(), client->CharacterID(),
					guild_mgr.GetGuildName(GuildID()), GuildID());
				...
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Sending OP_GuildInvite for invite to %s, length %d", client->GetName(), app->size);
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildInviteAccept");
	...
	if (IsInAGuild() && gj->response == GuildRank())
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Guild Invite Accept: guild %d, response %d, inviter %s, person %s",
		...
		if (gj->guildeqid == GuildID()) {
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Changing guild rank of %s (%d) to rank %d in guild %s (%d)",
				GetName(), CharacterID(),
				gj->response,
				guild_mgr.GetGuildName(GuildID()), GuildID());
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Adding %s (%d) to guild %s (%d) at rank %d",
				GetName(), CharacterID(),
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildLeader");

	if (app->size < 2) {
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Invalid length %d on OP_GuildLeader", app->size);
		return;
	}
	...
	if (!IsInAGuild())
		...
		if (newleader) {
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Transfering leadership of %s (%d) to %s (%d)",
				guild_mgr.GetGuildName(GuildID()), GuildID(),
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Got OP_GuildPeace of len %d", app->size);
	return;
}

void Client::Handle_OP_GuildRemove(const EQApplicationPacket *app)
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildRemove");
	...
	// we can always remove ourself, otherwise, our rank needs remove permissions
		...
		if (client) {
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Removing %s (%d) from guild %s (%d)",
				client->GetName(), client->CharacterID(),
				guild_mgr.GetGuildName(GuildID()), GuildID());
		}
		else {
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Removing remote/offline %s (%d) into guild %s (%d)",
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Got OP_GuildWar of len %d", app->size);
	return;
}
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::None, "%s sent a logout packet.", GetName());
...
void Client::Handle_OP_MoveItem(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Moveitem from_slot: %i, to_slot: %i, number_in_stack: %i", mi->from_slot, mi->to_slot, mi->number_in_stack);
	...
	if (IsValidSlot(mi->from_slot) && IsValidSlot(mi->to_slot)) {
		...
		if (!si)
		{
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "WTF Some shit failed. SwapItem: %i, IsValidSlot (from): %i, IsValidSlot (to): %i", SwapItem(mi), IsValidSlot(mi->from_slot), IsValidSlot(mi->to_slot));
			SwapItemResync(mi);
...
void Client::Handle_OP_RezzAnswer(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Received OP_RezzAnswer from client. Pendingrezzexp is %i, action is %s",
...
void Client::Handle_OP_Sacrifice(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(Sacrifice_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_Sacrifice expected %i got %i", sizeof(Sacrifice_Struct), app->size);
		DumpPacket(app);
		return;
	}
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_SetGuildMOTD");

	if (app->size != sizeof(GuildMOTD_Struct)) {
		// client calls for a motd on login even if they arent in a guild
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Error: app size of %i != size of GuildMOTD_Struct of %zu\n", app->size, sizeof(GuildMOTD_Struct));
		return;
	}
	if (!IsInAGuild()) {
	...
	if (gmotd->motd[0] == 0)
	{
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Client is trying to remove MOTD. This may be intentional but still will be prevented for now.");
		return; //EQMac sends this on login, and it overwrites the existing MOTD. Need to figure out a better way to handle it
	}

	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Setting MOTD for %s (%d) to: %s - %s",
...
void Client::Handle_OP_SetTitle(const EQApplicationPacket *app)
{
	if (app->size != sizeof(SetTitle_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_SetTitle expected %i got %i", sizeof(SetTitle_Struct), app->size);
		DumpPacket(app);
		return;
	}
...
#if EQDEBUG >= 11
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "%s, purchase item..", GetName());
	DumpPacket(app);
#endif
...
void Client::Handle_OP_Surname(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in Surname expected %i got %i", sizeof(Surname_Struct), app->size);
		return;
	}
	...
	if (app->size == sizeof(Trader_ShowItems_Struct))
	{
		...
		case BazaarTrader_EndTransaction: {
			...
			if (c)
				...
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::Handle_OP_Trader: Null Client Pointer");
		...
		default: {
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Unhandled action code in OP_Trader ShowItems_Struct");
			break;
		}
		}
	}
...
void Client::Handle_OP_TraderBuy(const EQApplicationPacket *app) 
{
	...
	if (app->size == sizeof(TraderBuy_Struct)){
		...
		if (Client* Trader = entity_list.GetClientByID(tbs->TraderID)){
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::Handle_OP_TraderBuy: Null Client Pointer");
		}
	}
	else {
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::Handle_OP_TraderBuy: Struct size mismatch");
...
void Client::Handle_OP_TraderShop(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(TraderClick_Struct)) {
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::Handle_OP_TraderShop: Returning due to struct size mismatch");
	...
	if (Trader)
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Client::Handle_OP_TraderShop: entity_list.GetClientByID(tcs->traderid)"
...
void Client::Handle_OP_Translocate(const EQApplicationPacket *app) 
{
	...
	if (app->size != sizeof(Translocate_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_Translocate expected %i got %i", sizeof(Translocate_Struct), app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_LFGCommand(const EQApplicationPacket *app)
{
	if (app->size != sizeof(LFG_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Invalid size for LFG_Struct: Expected: %i, Got: %i", sizeof(LFG_Struct), app->size);
		return;
	}
	...
	{
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::General, "Invalid LFG value sent. %i", lfg->value);
	}
