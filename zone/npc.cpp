...
void NPC::QueryLoot(Client* to) {
	...
	for (; cur != end; ++cur) {
		...
		if (item)
		{
			...
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Database error, invalid item");
		x++;
	}
...
uint32 ZoneDatabase::CreateNewNPCCommand(const char* zone, uint32 zone_version,Client *client, NPC* spawn, uint32 extra) {
	...
	if (npc_type_id)
	{
        ...
        auto results = QueryDatabase(query);
		if (!results.Success()) {
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "NPCSpawnDB Error: %s %s", query.c_str(), results.ErrorMessage().c_str());
			return false;
		}
...
uint32 ZoneDatabase::AddNewNPCSpawnGroupCommand(const char* zone, uint32 zone_version, Client *client, NPC* spawn, uint32 respawnTime) {
    ...
    auto results = QueryDatabase(query);
	if (!results.Success()) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "CreateNewNPCSpawnGroupCommand Error: %s %s", query.c_str(), results.ErrorMessage().c_str());
		return 0;
	}
...
void Mob::NPCSpecialAttacks(const char* parse, int permtag, bool reset, bool remove) {
	...
	if(permtag == 1 && this->GetNPCTypeID() > 0)
	{
		...
		{
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Normal, "NPCTypeID: %i flagged to '%s' for Special Attacks.\n",this->GetNPCTypeID(),orig_parse);
		}
	}
}
