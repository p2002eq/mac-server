...
float Mob::ResistSpell(uint8 resist_type, uint16 spell_id, Mob *caster, bool use_resist_override, int resist_override, bool CharismaCheck, bool CharmTick, bool IsRoot)
{
	...
	if (CharmTick) {
		...
		int min_charmbreakchance = (int)(((100 / (float)RuleI(Spells, CharmBreakCheckChance)) / 66 * 100) * 2);
	...
	if (IsRoot) {
		...
		int min_rootbreakchance = (int)(((100 / (float)RuleI(Spells, RootBreakCheckChance)) / 22 * 100) * 2);
