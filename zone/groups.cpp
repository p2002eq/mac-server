...
: GroupIDConsumer()
{
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::General, "Group:Group() Setting OldLeader to: %s and Leader to: %s", GetOldLeaderName(), leader->GetName());
	uint32 i;
	for(i=0;i<MAX_GROUP_MEMBERS;i++)
	{
		memset(membername[i],0,64);
...
bool Group::DelMemberOOZ(const char *Name, bool checkleader) {
	...
	for(unsigned int i = 0; i < MAX_GROUP_MEMBERS; i++) 
	{
		if(!strcasecmp(Name, membername[i]))
		{
			...
			if(!members[i]) {
				...
				MemberRoles[i] = 0;
				removed = true;
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::General, "DelMemberOOZ: Removed Member: %s", Name);
				break;
			}
		}
	}
	...
	if(checkleader)
	{
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::General, "DelMemberOOZ: Checking leader...");
...
bool Group::DelMember(Mob* oldmember,bool ignoresender)
{
	...
	for (uint32 i = 0; i < MAX_GROUP_MEMBERS; i++)
	{
		if (members[i] == oldmember)
		{
			...
			memset(membername[i],0,64);
			MemberRoles[i] = 0;
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::General, "DelMember: Removed Member: %s", oldmember->GetCleanName());
			break;
		}
	}
...
void Group::ChangeLeader(Mob* newleader)
{
	...
	for (uint32 i = 0; i < MAX_GROUP_MEMBERS; i++) {
		...
		{
			members[i]->CastToClient()->QueuePacket(outapp);
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::General, "ChangeLeader(): Local leader update packet sent to: %s .", members[i]->GetName());
		}
	}
	safe_delete(outapp);

	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::General, "ChangeLeader(): Old Leader is: %s New leader is: %s", GetOldLeaderName(), newleader->GetName());
