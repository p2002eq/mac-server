...
void Client::SetEXP(uint32 set_exp, uint32 set_aaxp, bool isrezzexp) {
	...
	if (admin>=100 && GetGM()) {
		...
		Message(CC_Yellow, "[GM] You have gained %d AXP and %d EXP (%d)", ConvertArray(set_aaxp,val1),ConvertArray(set_exp,val2),ConvertArray(GetEXPForLevel(GetLevel()+1),val3));
	}
}
