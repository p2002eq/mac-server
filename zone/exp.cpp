		...
		if (members[i] != nullptr && members[i]->IsClient()) // If Group Member is Client
		{
				...
				cmember->IsInRange(other))
			{
				if (cmember->IsInLevelRange(maxlevel)) 
				{
					...
					Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::General, "%s is too low in level to gain XP from this group.", cmember->GetName());
			}
			else
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::General, "%s is not in the kill zone, is out of range, or %s is green to them. They won't recieve group XP.", cmember->GetName(), other->GetCleanName());
		}
	}
}
