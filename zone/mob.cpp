...
void Mob::SendIllusionPacket(uint16 in_race, uint8 in_gender, uint8 in_texture, uint8 in_helmtexture, uint8 in_haircolor, uint8 in_beardcolor, uint8 in_eyecolor1, uint8 in_eyecolor2, uint8 in_hairstyle, uint8 in_luclinface, uint8 in_beard, uint8 in_aa_title, float in_size) {
	...
	safe_delete(outapp);
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Illusion: Race = %i, Gender = %i, Texture = %i, HelmTexture = %i, HairColor = %i, BeardColor = %i, EyeColor1 = %i, EyeColor2 = %i, HairStyle = %i, Face = %i, Size = %f",
...
bool Mob::DoKnockback(Mob *caster, float pushback, float pushup)
{
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "DoKnockback(): caster %s: target: %s pushback %0.1f pushup %0.1f", caster->GetCleanName(), GetCleanName(), pushback, pushup);
	float new_x = GetX();
	float new_y = GetY();
	...
	{
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Combat, "DoKnockback(): Old coords %0.2f,%0.2f,%0.2f New coords %0.2f,%0.2f,%0.2f ", GetX(), GetY(), GetZ(), new_x, new_y, new_z);
		m_Position.x = new_x;
		m_Position.y = new_y;
		m_Position.z = new_z;
	...
	{
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Combat, "DoKnockback(): LOS check failed.");
		return false;
	}
}
