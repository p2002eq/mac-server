...
void Client::ReportConnectingState() {
	...
	case PlayerProfileLoaded:	//our DB work is done, sending it
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "We were sending the player profile, spawns, time and weather, but never finished.");
		break;
...
bool Client::CheckIncreaseSkill(SkillUseTypes skillid, Mob *against_who, int chancemodi) {
	...
	if(against_who)
	{
		...
		{
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Skills, "Skill %d at value %d failed to gain due to invalid target.", skillid, skillval);
			return false; 
		}
	}
	...
	if (skillval < maxskill)
	{
		...
		if(zone->random.Real(0, 99) < Chance)
		{
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Skills, "Skill %d at value %d successfully gain with %i percent chance (mod %d)", skillid, skillval, Chance, chancemodi);
			return true;
		} else {
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Skills, "Skill %d at value %d failed to gain with %i percent chance (mod %d)", skillid, skillval, Chance, chancemodi);
		}
	} else {
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Skills, "Skill %d at value %d cannot increase due to maximum %d", skillid, skillval, maxskill);
	}
	return false;
}
