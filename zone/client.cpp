...
void Client::ReportConnectingState() {
	...
	case PlayerProfileLoaded:	//our DB work is done, sending it
		logger.LogDebug(EQEmuLogSys::General, "We were sending the player profile, spawns, time and weather, but never finished.");
		break;
	...
	case 8: { /* Say */
		...
		if (GetTarget() != 0 && GetTarget()->IsNPC()) {
			if(!GetTarget()->CastToNPC()->IsEngaged()) {
				...
				if(ComparativeDistanceNoZ(m_Position, GetTarget()->GetPosition()) <= 200 && !IsInvisible(this)) {
...
bool Client::CheckIncreaseSkill(SkillUseTypes skillid, Mob *against_who, int chancemodi) {
	...
	if (skillval < maxskill)
	{
		...
		if(zone->random.Real(0, 99) < Chance)
		{
			...
			logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Skills, "Skill %d at value %d successfully gain with %i percent chance (mod %d)", skillid, skillval, Chance, chancemodi);
			return true;
		} else {
			logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Skills, "Skill %d at value %d failed to gain with %i percent chance (mod %d)", skillid, skillval, Chance, chancemodi);
		}
	} else {
		logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Skills, "Skill %d at value %d cannot increase due to maximum %d", skillid, skillval, maxskill);
	}
	return false;
}
