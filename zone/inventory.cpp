...
bool Client::SwapItem(MoveItem_Struct* move_in) {
		...
		IsBankSlot(dst_slot_check))
	{
		...
		if(!banker || distance > USE_NPC_RANGE2)
		{
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Banker error");
			return false;
		}
	}
	...
	if (src_inst){
		...
		if (src_inst->GetCharges() > 0 && (src_inst->GetCharges() < (int16)move_in->number_in_stack || move_in->number_in_stack > src_inst->GetItem()->StackSize))
		{
			...
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Insufficent number in stack. Ignore this if on EQMac.");
			return false;
		}
	}
	if (dst_inst) {
	...
	&& RuleI(World, FVNoDropFlag) == 0 || RuleI(Character, MinStatusForNoDropExemptions) < Admin() && RuleI(World, FVNoDropFlag) == 2) {
		...
		WorldKick();
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "No drop hack.");
		return false;
	}
	...
	if (dst_slot_id >= EmuConstants::TRADE_BEGIN && dst_slot_id <= EmuConstants::TRADE_END) {
		if (src_slot_id != MainCursor) {
			Kick();
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Trading item no on cursor.");
			return false;
		}
		if (with) {
			...
			if (!m_inv[MainCursor]) {
				...
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Error: Cursor item not located on server!");
				return false;
			}
	...
	if (move_in->number_in_stack > 0) {
		...
		else if(!src_inst)
		{
			...
			if ((int8)move_in->number_in_stack >= src_inst->GetCharges()) {
				...
				if(!m_inv.SwapItem(src_slot_id, dst_slot_id)) { 
					Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Could not move entire stack from %d to %d with stack size %d. Dest empty.", src_slot_id, dst_slot_id, move_in->number_in_stack);
					return false; 
				}
		...
		if(!m_inv.SwapItem(src_slot_id, dst_slot_id)) {
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Destination slot is not valid for item %s from slot %d to slot %d", src_inst->GetItem()->Name, src_slot_id, dst_slot_id);
			return false;
		}
	...
	else
	{
		...
		if(src_slot_id == MainCursor && baginst && baginst->IsType(ItemClassContainer))
		{
			for (int16 trade_bag_slot = EmuConstants::GENERAL_BAGS_BEGIN + (dst_slot_id - EmuConstants::GENERAL_BEGIN) * EmuConstants::ITEM_CONTAINER_SIZE; 
				...
				if(inst)
				{
					Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Sending out packet for bagged item: %s in slot: %i bag slot: %i", inst->GetItem()->Name, trade_bag_slot, dst_slot_id);
