...
bool ZoneDatabase::PopulateZoneSpawnList(uint32 zoneid, LinkedList<Spawn2*> &spawn2_list, int16 version, uint32 repopdelay) {
    ...
    if (!results.Success()) {
        logger.Log(EQEmuLogSys::Error,"Error in PopulateZoneLists query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
    }
...
Spawn2* ZoneDatabase::LoadSpawn2(LinkedList<Spawn2*> &spawn2_list, uint32 spawn2id, uint32 timeleft) {
    ...
    auto results = QueryDatabase(query);
    if (!results.Success()) {
        logger.Log(EQEmuLogSys::Error,"Error in LoadSpawn2 query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
        return nullptr;
    }
...
bool ZoneDatabase::CreateSpawn2(Client *client, uint32 spawngroup, const char* zone, const xyz_heading& position, uint32 respawn, uint32 variance, uint16 condition, int16 cond_value)
{
    ...
    auto results = QueryDatabase(query);
    if (!results.Success()) {
        logger.Log(EQEmuLogSys::Error,"Error in CreateSpawn2 query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
    }
...
void SpawnConditionManager::UpdateDBEvent(SpawnEvent &event) {
	...
	if(!results.Success())
		logger.Log(EQEmuLogSys::Error,"Unable to update spawn event '%s': %s\n", query.c_str(), results.ErrorMessage().c_str());
...
void SpawnConditionManager::UpdateDBCondition(const char* zone_name, uint32 instance_id, uint16 cond_id, int16 value) {
    ...
    auto results = database.QueryDatabase(query);
	if(!results.Success())
		logger.Log(EQEmuLogSys::Error,"Unable to update spawn condition '%s': %s\n", query.c_str(), results.ErrorMessage().c_str());
...
bool SpawnConditionManager::LoadDBEvent(uint32 event_id, SpawnEvent &event, std::string &zone_name) {
    ...
    auto results = database.QueryDatabase(query);
    if (!results.Success()) {
		logger.Log(EQEmuLogSys::Error,"Error in LoadDBEvent query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
	}
...
bool SpawnConditionManager::LoadSpawnConditions(const char* zone_name, uint32 instance_id)
{
    ...
    auto results = database.QueryDatabase(query);
    if (!results.Success()) {
        logger.Log(EQEmuLogSys::Error,"Error in LoadSpawnConditions query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
    }
    ...
    results = database.QueryDatabase(query);
    if (!results.Success()) {
        logger.Log(EQEmuLogSys::Error,"Error in LoadSpawnConditions events query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
    }
    ...
    for (auto row = results.begin(); row != results.end(); ++row) {
        ...
        if(event.period == 0) {
            logger.Log(EQEmuLogSys::Error,"Refusing to load spawn event #%d because it has a period of 0\n", event.id);
            continue;
        }
