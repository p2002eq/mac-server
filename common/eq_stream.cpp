	...
	switch (p->opcode) {
		...
		case OP_SessionRequest: {
			...
			if (GetState()==ESTABLISHED) {
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, _L "Received OP_SessionRequest in ESTABLISHED state (%d)" __L, GetState());
...
EQOldStream::~EQOldStream()
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, "Killing EQOldStream");
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, "Killing outbound and inbound packet queue");
	RemoveData();
	SetState(CLOSED);
}
...
bool EQOldStream::ProcessPacket(EQOldPacket* pack, bool from_buffer)
{
	...
	if(pack->HDR.a2_Closing && pack->HDR.a6_Closing)
	{
		...
		if(state == ESTABLISHED) {
			//client initiated disconnect?
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, _L "Received OP_SessionDisconnect. Treating like a client-initiated disconnect." __L);
			_SendDisconnect();
			SetState(CLOSED);
		} else if(state == CLOSING) {
			//we were waiting for this anyways, ignore pending messages, send the reply and be closed.
	...
	else 
	{
		...
		if(app->GetRawOpcode() != 62272 && (app->GetRawOpcode() != 0 || app->Size() > 2)) //ClientUpdate
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, "Received old opcode - 0x%x size: %i", app->GetRawOpcode(), app->Size());
		if(app)
			OutQueue.push_back(app);
		return true;
	}
...
//	ack_req = true;	// It's broke right now, dont delete this line till fix it. =P
	...
	if(OpMgr == nullptr || *OpMgr == nullptr) {
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, _L "Packet enqueued into a stream with no opcode manager, dropping.");
		delete p;
		return;
	}
...
//	ack_req = true;	// It's broke right now, dont delete this line till fix it. =P
	...
	if(pack->emu_opcode != OP_MobUpdate && pack->emu_opcode != OP_MobHealth && pack->emu_opcode != OP_HPUpdate)
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, _L "Sending old opcode 0x%x" __L, opcode);
	MakeEQPacket(pack2, ack_req);
	delete pack;
	delete pack2;
}
...
void EQOldStream::OutboundQueueClear()
{
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, _L "Clearing outbound & resend queue" __L);
...
void EQOldStream::SetState(EQStreamState state) {
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, _L "Changing state from %d to %d" __L, pm_state, state);
	pm_state=state;
	MState.unlock();
}
...
EQStream::MatchState EQOldStream::CheckSignature(const EQStream::Signature *sig) {
	...
	if (!OutQueue.empty()) {
		...
		if(p == nullptr) {
			...
			//opcode matches, check length..
			if(p->size == sig->first_length) {
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, "[TRACE] %s:%d: First opcode matched 0x%x and length matched %d", long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()), sig->first_eq_opcode, p->size);
				res = EQStream::MatchState::MatchSuccessful;
			} else if(sig->first_length == 0) {
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, "[TRACE] %s:%d: First opcode matched 0x%x and length (%d) is ignored", long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()), sig->first_eq_opcode, p->size);
				res = EQStream::MatchState::MatchSuccessful;
			} else {
				//opcode matched but length did not.
				Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, "[TRACE] %s:%d: First opcode matched 0x%x, but length %d did not match expected %d", long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()), sig->first_eq_opcode, p->size, sig->first_length);
				res = EQStream::MatchState::MatchFailed;
			}
		} else {
			//first opcode did not match..
			Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, "[TRACE] %s:%d: First opcode 0x%x did not match expected 0x%x", long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()), p->GetRawOpcode(), sig->first_eq_opcode);
			res = EQStream::MatchState::MatchFailed;
		}
	}
	MInboundQueue.unlock();
...
void EQOldStream::Close() {
	if(HasOutgoingData()) {
		...
		//otherwise, we are done, we can drop immediately.
		_SendDisconnect();
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, _L "EQStream closing immediate due to Close()" __L);
		SetState(DISCONNECTING);
	}
}
...
{
	EQRawApplicationPacket *ap=nullptr;
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Netcode, _L "Creating old application packet, length %d" __L, p->dwExtraSize);
	ap = p->MakeAppPacket();
	return ap;
}
