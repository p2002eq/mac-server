...
namespace Mac {
	...
	ENCODE(OP_PlayerProfile) 
	{
		...
		//Log.Out(EQEmuLogSys::General, EQEmuLogSys::Netcode, "[STRUCTS] Player Profile Packet is %i bytes uncompressed", sizeof(structs::PlayerProfile_Struct));
		...
		//Log.Out(EQEmuLogSys::General, EQEmuLogSys::Netcode, "[STRUCTS] Player Profile Packet is %i bytes compressed", outapp->size);
		...
		*p = nullptr;
		...
		if(item)
		{
			...
			if(outapp->size != sizeof(structs::Item_Struct))
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Zone_Server, "Invalid size on OP_ItemPacket packet. Expected: %i, Got: %i", sizeof(structs::Item_Struct), outapp->size);
		...
		*p = nullptr;
		...
		if(item)
		{
			...
			if(outapp->size != sizeof(structs::TradeItemsPacket_Struct))
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Zone_Server, "Invalid size on OP_TradeItemPacket packet. Expected: %i, Got: %i", sizeof(structs::TradeItemsPacket_Struct), outapp->size);
	...
	ENCODE(OP_CharInventory)
	{
		...
		*p = nullptr;
		...
		{
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Netcode, "[STRUCTS] Wrong size on outbound %s: Got %d, expected multiple of %d", opcodes->EmuToName(in->GetOpcode()), in->size, sizeof(InternalSerializedItem_Struct));
			delete in;
			return;
		}
	...
	ENCODE(OP_ShopInventoryPacket)
	{
		...
		*p = nullptr;
		...
		if(itemcount == 0 || (in->size % sizeof(InternalSerializedItem_Struct)) != 0) 
		{
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Zone_Server, "Wrong size on outbound %s: Got %d, expected multiple of %d", opcodes->EmuToName(in->GetOpcode()), in->size, sizeof(InternalSerializedItem_Struct));
			delete in;
			return;
		}
		if(itemcount > 79)
	...
	DECODE(OP_MoveItem)
	{
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "EQMAC DECODE OUTPUT to_slot: %i, from_slot: %i, number_in_stack: %i", emu->to_slot, emu->from_slot, emu->number_in_stack);
		FINISH_DIRECT_DECODE();
	}
	...
	ENCODE(OP_MoveItem)
	{
		...
		OUT(to_slot);
		OUT(number_in_stack);
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "EQMAC ENCODE OUTPUT to_slot: %i, from_slot: %i, number_in_stack: %i", eq->to_slot, eq->from_slot, eq->number_in_stack);
	...
	ENCODE(OP_GroundSpawn) 
	{
		...
		for(g=0; g<10; g++)
		{
			if(eq->itemsinbag[g] > 0)
			{
				...
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Inventory, "Found a container item %i in slot: %i", emu->itemsinbag[g], g);
			}
			else
	...
	ENCODE(OP_Unknown)
	{
		...
		*p = nullptr;
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Client_Server_Packet, "Dropped an invalid packet: %s", opcodes->EmuToName(in->GetOpcode()));
