...
bool Database::CharacterJoin(uint32 char_id, char* char_name) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "CharacterJoin should have wrote to database for %s with ID %i at %i and last_seen should be zero.", char_name, char_id, time(nullptr));
...
bool Database::CharacterQuit(uint32 char_id) {
	...
	auto results = QueryDatabase(query);
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "Loading EQ time of day failed. Using defaults.");
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "CharacterQuit should have wrote to database for %i at %i", char_id, time(nullptr));
	if (!results.Success()){
		LogFile->write(EQEmuLog::Debug, "Error updating character_data table from CharacterQuit.");
		return false;
	}
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "Loading EQ time of day failed. Using defaults.");
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "CharacterQuit should have wrote to database for %i...", char_id);
	return true;
}
...
bool Database::ZoneConnected(uint32 id, const char* name) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "Loading EQ time of day failed. Using defaults.");
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "ZoneConnected should have wrote id %i to webdata_servers for %s with connected status 1.", id, name);
...
bool Database::ZoneDisconnect(uint32 id) {
	...
	auto results = QueryDatabase(query);
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "Loading EQ time of day failed. Using defaults.");
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "ZoneDisconnect should have wrote '0' to webdata_servers for %i.", id);
	if (!results.Success()){
...
bool Database::LSConnected(uint32 port) {
	...
	auto connect_results = QueryDatabase(connect_query);
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "Loading EQ time of day failed. Using defaults.");
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "LSConnected should have wrote id %i to webdata_servers for LoginServer with connected status 1.", port);
...
bool Database::LSDisconnect() {
	...
	auto results = QueryDatabase(query);
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::General, "LSConnected should have wrote to webdata_servers for LoginServer connected status 0.");
	if (!results.Success()){
...
void Database::SetGroupLeaderName(uint32 gid, const char* name) { 
	...
	if (!results.Success())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Unable to set group leader:", results.ErrorMessage().c_str());
}
...
struct TimeOfDay_Struct Database::LoadTime(time_t &realtime)
{
	...
	if (!results.Success() || results.RowCount() == 0)
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Loading EQ time of day failed. Using defaults.");
		eqTime.minute = 0;
		eqTime.hour = 9;
		eqTime.day = 1;
		eqTime.month = 1;
