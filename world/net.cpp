...
//	register_remote_call_handlers();
	...
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Scanning Database setup for updates..");
	database.DBSetup();
	...
	while(RunLoops) {
		...
		while ((eqos = eqsf.PopOld())) {
			...
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "New connection from %s:%d", inet_ntoa(in), ntohs(eqos->GetRemotePort()));
...
void CatchSignal(int sig_num) {
	...
	if(!database.SaveTime(eqTime.minute,eqTime.hour,eqTime.day,eqTime.month,eqTime.year))
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Failed to save eqtime.");
	RunLoops = false;
}
