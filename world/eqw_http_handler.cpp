...
void EQWHTTPServer::Run() {
	logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "HTTP Processing thread started on port %d", m_port);
	do {
#warning DELETE THIS IF YOU DONT USE IT
		Sleep(10);
	} while(m_running);
	logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "HTTP Processing thread terminating on port %d", m_port);
}
