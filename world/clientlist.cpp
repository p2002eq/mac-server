...
void ClientList::SendWhoAll(uint32 fromid,const char* to, int16 admin, Who_All_Struct* whom, WorldTCPConnection* connection) {
	...
	safe_delete(output);
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Unknown error in world's SendWhoAll (probably mem error), ignoring... Player id is: %i, Name is: %s", fromid, to);
		return;
	}
}
