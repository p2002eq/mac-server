		...
		switch(pack->opcode) {
			...
			case ServerOP_ZoneToZoneRequest: {
				...
				if(GetZoneID() == ztz->current_zone_id && GetInstanceID() == ztz->current_instance_id) {
					logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Processing ZTZ for egress from zone for client %s", ztz->name);
					...
					if(ingress_server) {
						logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Found a zone already booted for %s", ztz->name);
						ztz->response = 1;
					}
					/* Boot the Zone*/
					else {
						...
						if ((server_id = zoneserver_list.TriggerBootup(ztz->requested_zone_id, ztz->requested_instance_id))){
							logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Successfully booted a zone for %s", ztz->name);
							// bootup successful, ready to rock
							ztz->response = 1;
							ingress_server = zoneserver_list.FindByID(server_id);
						}
						else {
							logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "FAILED to boot a zone for %s", ztz->name);
				...
				/* Response from Ingress server, route back to egress */
					...
					logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Processing ZTZ for ingress to zone for client %s", ztz->name);
			...
			case ServerOP_RequestOnlineGuildMembers: {
				...
				logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "ServerOP_RequestOnlineGuildMembers Recieved. FromID=%i GuildID=%i", srogms->FromID, srogms->GuildID);
