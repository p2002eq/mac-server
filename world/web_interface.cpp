...
void WebInterfaceConnection::SetConnection(EmuTCPConnection *inStream)
{
	if(stream)
	{
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::WebInterface_Server, "Incoming WebInterface Connection while we were already connected to a WebInterface.");
		stream->Disconnect();
	}
...
bool WebInterfaceConnection::Process()
{
	...
	while((pack = stream->PopPacket()))
	{
		if (!authenticated)
		{
			if (WorldConfig::get()->SharedKey.length() > 0)
			{
				if (pack->opcode == ServerOP_ZAAuth && pack->size == 16)
				{
					...
					else
					{
						struct in_addr in;
						in.s_addr = GetIP();
						Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::WebInterface_Server, "WebInterface authorization failed.");
			...
			{
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::WebInterface_Server, "**WARNING** You have not configured a world shared key in your config file. You should add a <key>STRING</key> element to your <world> element to prevent unauthorized zone access.");
				authenticated = true;
			}
			delete pack;
			continue;
		...
		switch(pack->opcode)
		{
			...
			{
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::WebInterface_Server, "Got authentication from WebInterface when they are already authenticated.");
				break;
			}
			...
			default:
			{
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::WebInterface_Server, "Unknown ServerOPcode from WebInterface 0x%04x, size %d", pack->opcode, pack->size);
