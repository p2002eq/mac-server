...
void EQW::AppendOutput(const char *str) {
	m_outputBuffer += str;
//	logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Append %d chars, yeilding result of length %d", strlen(str), m_outputBuffer.length());
}

const std::string &EQW::GetOutput() const {
//	logger.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Getting, length %d", m_outputBuffer.length());
