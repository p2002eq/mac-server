...
void Console::Die() {
	...
	struct in_addr in;
	in.s_addr = GetIP();
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Removing console from %s:%d",inet_ntoa(in),GetPort());
	tcpc->Disconnect();
}
...
bool Console::Process() {
	...
	if (!tcpc->Connected()) {
		struct in_addr in;
		in.s_addr = GetIP();
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Removing console (!tcpc->Connected) from %s:%d",inet_ntoa(in),GetPort());
		return false;
	}
	...
	if (timeout_timer.Check()) {
		...
		struct in_addr in;
		in.s_addr = GetIP();
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"TCP connection timeout from %s:%d",inet_ntoa(in),GetPort());
		return false;
	}
	...
	if (tcpc->GetMode() == EmuTCPConnection::modePacket) {
		...
		if(tcpc->GetPacketMode() == EmuTCPConnection::packetModeZone) {
			...
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"New zoneserver #%d from %s:%d", zs->GetID(), inet_ntoa(in), GetPort());
			zoneserver_list.Add(zs);
			numzones++;
			tcpc = 0;
		} else if(tcpc->GetPacketMode() == EmuTCPConnection::packetModeLauncher) {
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"New launcher from %s:%d", inet_ntoa(in), GetPort());
			launcher_list.Add(tcpc);
			tcpc = 0;
		} 
		else if(tcpc->GetPacketMode() == EmuTCPConnection::packetModeUCS)
		{
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"New UCS Connection from %s:%d", inet_ntoa(in), GetPort());
			UCSLink.SetConnection(tcpc);
			tcpc = 0;
		}
		else if(tcpc->GetPacketMode() == EmuTCPConnection::packetModeQueryServ)
		{
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"New QS Connection from %s:%d", inet_ntoa(in), GetPort());
...
/*		else if (tcpc->GetPacketMode() == EmuTCPConnection::packetModeWebInterface)
		{
			...
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Unsupported packet mode from %s:%d", inet_ntoa(in), GetPort());
		}
		return false;
	}
	char* command = 0;
...
void Console::ProcessCommand(const char* command) {
	switch(state)
	{
		...
		case CONSOLE_STATE_PASSWORD:
		{
			...
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"TCP console authenticated: Username=%s, Admin=%d",paccountname,admin);
			SendMessage(1, 0);
		...
		case CONSOLE_STATE_CONNECTED: {
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"TCP command: %s: \"%s\"",paccountname,command);
...
// SCORPIOUS2K - reversed parameter order for flag
				...
				if (sep.arg[2][0] == 0 || !sep.IsNumber(1)) {
					...
					Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Console ZoneBootup: %s, %s, %s",tmpname,sep.arg[2],sep.arg[1]);
				...
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Login Server Reconnect manually restarted by Console");
			}
